...
// OwnDevicesMessage (PATCH pjgfactsalud, 25-09-2026): si no es nil, decide
// qué mensaje reciben TUS OTROS dispositivos como copia de uno enviado desde
// éste (el DeviceSentMessage). Devolver nil deja la copia idéntica, que es lo
// de siempre. Existe porque la app de WhatsApp de una cuenta personal no pinta
// en tu propio teléfono las tarjetas interactivas (botones, listas) que
// enviaste desde un dispositivo vinculado: el destinatario las ve, tú ves un
// hueco. Con esto se te manda el texto de respaldo en su lugar.
var OwnDevicesMessage func(to types.JID, message *waE2E.Message) *waE2E.Message
...
func marshalMessage(to types.JID, message *waE2E.Message) (plaintext, dsmPlaintext []byte, err error) {
	...
	if to.Server != types.GroupServer && to.Server != types.NewsletterServer {
		propio := message
		if OwnDevicesMessage != nil {
			if alt := OwnDevicesMessage(to, message); alt != nil {
				propio = alt
			}
		}
